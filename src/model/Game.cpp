...
Game::Game() :
		currentPlayer(nullptr), player1(nullptr), player2(nullptr), winner(-1) {
...
void draughts::model::Game::initGame(int pnum1, std::string pname1, int pnum2,
		std::string pname2) {
	getInstance();
...
bool draughts::model::Game::checkGameOver() {
	/* If there are not more posssible moves or jumps for this player then end the game*/
	if (!currentPlayer->possibleJumps() && !currentPlayer->possibleMoves()) {
		//Cannot make a move or a jump, have either lost or it might be a draw
		//Swap players to check again, in either case return true as the game is over
		swapPlayers();
		winner = currentPlayer->getPlayerNum();
		return true;
	}
	return false;
}
